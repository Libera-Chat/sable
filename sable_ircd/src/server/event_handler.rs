use super::*;
use crate::capability::TaggableMessage;
...
impl ClientServer
{
    ...
    fn handle_umode_change(&mut self, details: &update::UserModeChange) -> HandleResult
    {
        if let Ok(conn) = self.connections.get_user(details.user.user.id)
        {
            conn.send(&message::Mode::new(&details.changed_by,
                                          &details.user,
                                          &format_umode_changes(&details.added, &details.removed)));
        }
        Ok(())
    }
    ...
    {
        let (mut changes, params) = format_cmode_changes(detail);
        for p in params
        {
            changes.push(' ');
            changes.push_str(&p);
        ...
        self.send_to_channel_members(&wrapper::Channel::wrap(&*self.network(), &detail.channel), msg);
    ...
    {
        let net = self.network();
        let chan = net.channel(detail.channel.id)?;
    ...
    {
        let (mut changes, args) = format_channel_perm_changes(&detail.user.nickname, &detail.added, &detail.removed);
    ...
    {
        let net = self.network();
        let membership = net.membership(detail.membership.id)?;
    ...
    {
        let net = self.network();
        let source = net.user(detail.membership.user)?;
        ...
        let channel = net.channel(detail.membership.channel)?;
    ...
    {
        let net = self.network();
        let channel = net.channel(detail.channel.id)?;
    ...
    fn handle_new_message(&self, detail: &update::NewMessage) -> HandleResult
    {
        ...
        match &detail.target {
            update::HistoricMessageTarget::Channel(channel) => {
                self.send_to_channel_members_where(&wrapper::Channel::wrap(&*self.network(), &channel), message_send,
            ...
            update::HistoricMessageTarget::User(user) => {
                self.send_to_user_if_local(&wrapper::User::wrap(&*self.network(), &user.user), message_send);
            },
    ...
    fn handle_new_server(&self, detail: &update::NewServer) -> HandleResult
    {
        ...
        self.server.sync_log().enable_server(*server.name(), server.id());
    ...
    fn handle_server_quit(&self, detail: &update::ServerQuit) -> HandleResult
    {
        ...
        if detail.server.id == self.server.id() && detail.server.epoch == self.server.epoch()
        {
            // The network thinks we're no longer alive. Shut down to avoid desyncs
            panic!("Network thinks we're dead. Making it so");
        }

        self.server.sync_log().disable_server(detail.server.name, detail.server.id, detail.server.epoch);
    ...
    {
        let net = self.network();
        let entry = net.audit_entry(detail.entry.id)?;
        ...
        for conn in self.connections.iter()
        {
            ...
            {
                if let Ok(user) = net.user(user_id)
                {
                    if user.is_oper()
                    {
