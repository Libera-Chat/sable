...
pub(super) async fn handle_login(source: UserId, cmd: Arc<ClientCommand>) -> CommandResult
{
    ...
    let (username, password) = match cmd.args.len()
    {
        ...
        2 => {
            (net.user(source)?.nick(), cmd.args[1].clone())
        }
        _ => {
    ...
    match login_result
    {
        Ok(RemoteServerResponse::LogUserIn(account)) => {
            ...
            {
                cmd.new_event(source, UserLogin { account: Some(account) });
