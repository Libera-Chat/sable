...
async fn handle_privmsg(server: &ClientServer, source: UserSource<'_>, cmd: &dyn Command,
    target: TargetParameter<'_>, msg: &str) -> CommandResult
{
    if let Some(user) = target.user()
    {
        if let Some(alias) = user.is_alias_user()
        {
            return super::services::dispatch_alias_command(cmd, &user, &alias.command_alias, msg).await;
        }
    }
