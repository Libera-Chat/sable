...
command_handler!("OPER" => OperHandler {
    ...
    fn handle_user(&mut self, source: &wrapper::User, cmd: &ClientCommand) -> CommandResult
    {
        ...
        if let Some(conf) = self.find_oper_block(source, oper_name)
        {
...
impl OperHandler<'_>
{
    fn find_oper_block<'a>(&'a self, _user: &wrapper::User, oper_name: &str) -> Option<&'a config::OperConfig>
    {
        let conf = self.server.network().config();
