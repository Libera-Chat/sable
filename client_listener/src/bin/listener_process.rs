...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (control_recv, event_send) = unsafe {
        (
            IpcReceiver::<ControlMessage>::from_raw_fd(
                control_fd,
                client_listener::MAX_CONTROL_SIZE,
            )
            .expect("Failed to unpack control receiver"),
            IpcSender::<InternalConnectionEvent>::from_raw_fd(
                event_fd,
                client_listener::MAX_MSG_SIZE,
            )
            .expect("Failed to unpack event sender"),
        )
    };
