...
impl RegistrationPolicyService for StandardRegistrationPolicy {
    fn can_view_access(
        ...
        let source_access = source_account
            .has_access_in(channel.id())
            .ok_or(RegistrationPermissionError::NoAccess)?;

        if !source_access
            .role()?
            .flags()
            .is_set(ChannelAccessFlag::AccessView)
        {
    ...
    fn can_view_roles(
        ...
        if !source_access
            .role()?
            .flags()
            .is_set(ChannelAccessFlag::RoleView)
        {
            return Err(RegistrationPermissionError::NoAccess.into());
        }
    ...
    fn can_change_access_for(
        ...
        chan: &wrapper::ChannelRegistration,
        target: &wrapper::Account,
    ) -> PermissionResult {
        let source_access = source
            .has_access_in(chan.id())
            .ok_or(RegistrationPermissionError::NoAccess)?;

        if !source_access
            .role()?
            .flags()
            .is_set(ChannelAccessFlag::AccessEdit)
        {
            return Err(RegistrationPermissionError::NoAccess.into());
        }
        ...
        if let Some(current_flags) =
            target_access.and_then(|access| access.role().ok().map(|r| r.flags()))
        {
    ...
    fn can_edit_role(
        ...
        if !source_access
            .role()?
            .flags()
            .is_set(ChannelAccessFlag::RoleEdit)
        {
            return Err(RegistrationPermissionError::NoAccess.into());
        }
