use crate::{id::*, types::Pattern, validated::*};
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if let Ok(ip) = std::net::IpAddr::from_str(s) {
                    ...
                    .all(|c| ipv4_chars.contains(c))
            {
                ...
                let ip_addr =
                    std::net::Ipv4Addr::new(numbers[0], numbers[1], numbers[2], numbers[3]);
            ...
            Ok(Self::ExactHostname(
                s.try_into().map_err(|_| InvalidBanMask)?,
            ))
        }
    }
}
...
impl NetworkBanHostMatch {
    pub fn matches(&self, user_details: &UserDetails) -> bool {
        match self {
            NetworkBanHostMatch::ExactIp(ip) => user_details.ip == Some(ip),
            ...
            NetworkBanHostMatch::ExactHostname(host) => user_details.host == Some(host.as_ref()),
...
impl NetworkBanMatch {
    pub fn from_user_host(user: &str, host: &str) -> Result<Self, InvalidBanMask> {
        let ident_match = if user == "*" {
            None
        } else {
            Some(Pattern::new(user.to_owned()))
        };
    ...
    pub fn matches(&self, user_details: &UserDetails) -> bool {
        ...
        if let (Some(realname), Some(user_realname)) =
            (self.realname.as_ref(), user_details.realname)
        {
