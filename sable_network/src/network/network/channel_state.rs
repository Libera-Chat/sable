...
impl Network {
    ...
    pub(super) fn channel_mode_change(
        &mut self,
        target: ChannelId,
        event: &Event,
        details: &details::ChannelModeChange,
        updates: &dyn NetworkUpdateReceiver,
    ) {
    ...
    pub(super) fn new_channel_topic(
        &mut self,
        target: ChannelTopicId,
        event: &Event,
        details: &details::NewChannelTopic,
        updates: &dyn NetworkUpdateReceiver,
    ) {
        if let Some(existing) = self
            .channel_topics
            .values()
            .find(|t| t.channel == details.channel)
        {
            ...
            if existing.timestamp > event.timestamp
                || (existing.timestamp == event.timestamp && existing.id < target)
            {
    ...
    pub(super) fn new_list_mode_entry(
        &mut self,
        target: ListModeEntryId,
        event: &Event,
        details: &details::NewListModeEntry,
        updates: &dyn NetworkUpdateReceiver,
    ) {
    ...
    pub(super) fn del_list_mode_entry(
        &mut self,
        target: ListModeEntryId,
        event: &Event,
        details: &details::DelListModeEntry,
        updates: &dyn NetworkUpdateReceiver,
    ) {
    ...
    pub(super) fn channel_permission_change(
        &mut self,
        target: MembershipId,
        event: &Event,
        details: &details::MembershipFlagChange,
        updates: &dyn NetworkUpdateReceiver,
    ) {
    ...
    pub(super) fn user_joined_channel(
        &mut self,
        target: MembershipId,
        event: &Event,
        details: &details::ChannelJoin,
        updates: &dyn NetworkUpdateReceiver,
    ) {
        let membership =
            state::Membership::new(target, details.user, details.channel, details.permissions);
        ...
        if let Some(chan) = self.channels.remove(&id) {
            ...
            for list_type in ListModeType::iter() {
                ...
                self.list_mode_entries
                    .retain(|_id, entry| entry.list != list_id);
            }
        }
