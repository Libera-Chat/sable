...
impl Network {
    ...
    fn collide_user(&mut self, user_id: UserId, from: Nickname, trigger: &Event, updates: &dyn NetworkUpdateReceiver)
    {
        if let Some(user) = self.users.get(&user_id)
        {
            ...
            else
            {
                ...
                let new_binding = state::NickBinding::new(new_nick, user_id, trigger.timestamp, trigger.id);
    ...
    fn do_bind_nickname(&mut self, target: NicknameId, user: UserId, old_nick: Nickname, event: &Event, updates: &dyn NetworkUpdateReceiver)
    {
