...
impl<Policy: crate::policy::PolicyService> Server<Policy>
{
    ...
    fn handle_umode_change(&self, entry: &HistoryLogEntry, detail: &update::UserModeChange) -> HandleResult
    {
    ...
    fn handle_bulk_quit(&self, history_guard: &RwLockReadGuard<NetworkHistoryLog>, entry: &HistoryLogEntry, detail: &update::BulkUserQuit) -> HandleResult
    {
        for item in &detail.items
        {
            let new_entry = history_guard.add(NetworkStateChange::UserQuit(item.clone()), entry.timestamp);
            self.handle_user_quit(new_entry, item)?;
        }
        Ok(())
    }

    fn handle_channel_mode_change(&self, entry: &HistoryLogEntry, detail: &update::ChannelModeChange) -> HandleResult
    {
    ...
    fn handle_list_mode_added(&self, entry: &HistoryLogEntry, detail: &update::ListModeAdded) -> HandleResult
    {
        let network = self.network();
        let channel = wrapper::Channel::wrap(&*network, &detail.channel);

        self.notify_channel_members_where(&channel, entry,
                |m| self.policy_service.should_see_list_change(m, detail.list_type)
        );

        Ok(())
    }

    fn handle_list_mode_removed(&self, entry: &HistoryLogEntry, detail: &update::ListModeRemoved) -> HandleResult
    {
        let network = self.network();
        let channel = wrapper::Channel::wrap(&*network, &detail.channel);
    ...
    fn handle_channel_topic(&self, entry: &HistoryLogEntry, detail: &update::ChannelTopicChange) -> HandleResult
    {
        let network = self.network();
        let channel = wrapper::Channel::wrap(&*network, &detail.channel);
    ...
    fn handle_chan_perm_change(&self, entry: &HistoryLogEntry, detail: &update::MembershipFlagChange) -> HandleResult
    {
        let network = self.network();
        let channel = wrapper::Channel::wrap(&*network, &detail.channel);
    ...
    fn handle_invite(&self, entry: &HistoryLogEntry, detail: &update::ChannelInvite) -> HandleResult
    {
        self.notify_user(detail.user.user.id, entry.id);
    ...
    fn handle_channel_rename(&self, entry: &HistoryLogEntry, detail: &update::ChannelRename) -> HandleResult
    {
        let network = self.network();
        let channel = wrapper::Channel::wrap(&*network, &detail.channel);
    ...
    fn handle_new_message(&self, entry: &HistoryLogEntry, detail: &update::NewMessage) -> HandleResult
    {
        match &detail.target {
    ...
    fn handle_new_server(&self, entry: &HistoryLogEntry, detail: &update::NewServer) -> HandleResult
    {
    ...
    #[tracing::instrument(skip(self))]
    fn handle_server_quit(&self, entry: &HistoryLogEntry, detail: &update::ServerQuit) -> HandleResult
    {
        ...
        self.sync_log().disable_server(detail.server.name, detail.server.id, detail.server.epoch);

        Ok(())
    }

    fn report_audit_entry(&self, _entry: &HistoryLogEntry, _detail: &update::NewAuditLogEntry) -> HandleResult
    {
        Ok(())
    }
}
...
impl<Policy: crate::policy::PolicyService> NetworkUpdateReceiver for Server<Policy>
{
    fn notify_update(&self, update: NetworkStateChange, source_event: &Event)
    {
        ...
        if let Err(e) = self.subscriber.send(NetworkHistoryUpdate::NewEntry(entry.id))
        {
